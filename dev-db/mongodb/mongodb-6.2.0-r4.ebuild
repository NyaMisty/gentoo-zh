...
src_configure() {
        scons_opts=(
                ...
                --use-system-boost
                --use-system-pcre2 # use pcre2 instead of pcre, because parameter changed
                --use-system-snappy
                --use-system-stemmer
                --use-system-yaml
                --use-system-zlib
...
src_compile() {
        # because some packages (like requirements_parser) are not available on gentoo, 
        #  we install it into temporary location $MONGO_ROOT/site_scons, which will be 
        #  added to python path by ./buildscripts/scons.py
